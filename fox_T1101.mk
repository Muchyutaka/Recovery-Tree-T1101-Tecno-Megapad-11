...
#
# IMPORTANT: this tree builds the recovery into vendor_boot (GKI, virtual A/B):
#   lunch fox_T1101-eng && mka adbd vendorbootimage
# The kernel is NOT built/flashed - the device keeps its stock boot.img, so the
# modules in recovery/root/lib/modules must match the on-device kernel
# (see BOOTLOOP_ANALYSIS.md).
...
# Build fingerprint overrides (T1101 / t1101_m1101 firmware)
# Confirmed device build: T1101-M1101ABCD-U-OP-260410V1046; the tree's modules/
# DTB/fstab were verified byte-identical to that build's stock vendor_boot.
